...
if [ "$1" == "--inc" ]; then
    ...
    notify-send -h string:x-canonical-private-synchronous:spotify_volume "Spotify Volume" "Volume Increased to $(echo "$new_volume * 100" | bc)%"
elif [ "$1" == "--dec" ]; then
    ...
    notify-send -h string:x-canonical-private-synchronous:spotify_volume "Spotify Volume" "Volume Decreased to $(echo "$new_volume * 100" | bc)%"
else
